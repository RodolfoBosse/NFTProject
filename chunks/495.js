...
} = _metaplex_js__WEBPACK_IMPORTED_MODULE_2__.programs;
...
async function getNftsForOwner(connection, ownerAddress) {
  const allMintsCandyMachine = await (0,_hooks_use_hash_table__WEBPACK_IMPORTED_MODULE_5__/* .fetchHashTable */ .T$)("7vbWPbdPMPkf9B913RnKdQyEDWhZVtF89r1hQxn9Bfja");
  const allTokens = [];
  ...
  for (let index = 0; index < tokenAccounts.value.length; index++) {
    ...
    if (tokenAmount.amount == "1" && tokenAmount.decimals == "0" && allMintsCandyMachine.includes(tokenAccount.account.data.parsed.info.mint)) {
      ...
      const metadata = new Metadata(ownerAddress.toString(), accountInfo.value);
      const {
        data
